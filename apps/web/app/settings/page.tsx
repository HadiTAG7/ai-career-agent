...
export default function SettingsPage() {
  ...
  return (
    <div className="page-wrap page-enter max-w-[900px]">
      ...
      <section className="open-section mt-8"><h2 className="section-title flex items-center gap-2"><Database className="h-5 w-5 text-emerald" />{locale === "ar" ? "اتصال البيانات" : "Data connection"}</h2><p className="mt-3 text-sm text-muted">{locale === "ar" ? "الوضع الحالي" : "Current mode"}: <code dir="ltr" className="rounded bg-slate-100 px-2 py-1">{apiConfiguration.mode}</code></p><p className="mt-2 text-xs text-muted">{apiConfiguration.baseUrl ? (locale === "ar" ? "الوضع متصل بالخادم ويفشل بأمان عند انقطاعه؛ لا تُستبدل بياناتك بنتائج تجريبية." : "Connected mode fails closed during an outage; your data is never replaced with demo results.") : (locale === "ar" ? "لا يوجد خادم مضبوط؛ تعمل الواجهة كساحة تجريبية محلية موسومة بوضوح." : "No server is configured; the interface runs as a clearly labeled local demo sandbox.")}</p></section>
      <section className="open-section"><h2 className="section-title flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-emerald" />{locale === "ar" ? "الخصوصية" : "Privacy"}</h2><label className="mt-4 flex min-h-14 items-center justify-between gap-5"><span><strong className="block text-sm">{locale === "ar" ? "تحليلات استخدام اختيارية" : "Optional usage analytics"}</strong><small className="text-muted">{locale === "ar" ? "مغلقة افتراضيًا في هذا النموذج." : "Off by default in this prototype."}</small></span><input className="h-5 w-5 accent-emerald" type="checkbox" checked={analytics} onChange={(event) => setAnalytics(event.target.checked)} /></label></section>
      <section className="open-section"><h2 className="section-title">{locale === "ar" ? "بيانات المساحة المهنية" : "Career workspace data"}</h2><p className="mt-2 text-sm text-muted">{apiConfiguration.baseUrl ? (locale === "ar" ? "التصدير والحذف ينفذان عبر FastAPI وبجلسة تسجيل الدخول الحالية. الحذف يزيل بيانات المساحة المهنية فقط؛ حساب Clerk والجلسة يبقيان إلى أن ندمج حذف حساب الدخول قبل الإنتاج." : "Export and deletion run through FastAPI using your current authenticated session. Deletion removes career workspace data only; the Clerk account and session remain until account deletion is integrated before production.") : (locale === "ar" ? "وضع تجريبي: الحذف يمسح الحالة المحلية فقط، والتصدير الحقيقي غير متاح." : "Demo mode: deletion clears local state only, and real export is unavailable.")}</p><div className="mt-4 flex flex-wrap gap-3"><Button variant="secondary" disabled={busy} onClick={() => { void handleExport(); }}><Download className="h-4 w-4" />{locale === "ar" ? "تصدير بياناتي" : "Export my data"}</Button><Button variant="danger" disabled={busy} onClick={() => { void handleDelete(); }}><Trash2 className="h-4 w-4" />{apiConfiguration.baseUrl ? (locale === "ar" ? "حذف بيانات المساحة المهنية" : "Delete workspace data") : (locale === "ar" ? "مسح بيانات النموذج" : "Clear demo data")}</Button></div></section>
      {notice ? <p className="mt-4 rounded-lg border border-emerald bg-emerald-pale p-3 text-sm" role="status">{notice}</p> : null}
    </div>
  );
}
