...
import { CheckCircle2, Clipboard, Download, FileCheck2, FileText, LockKeyhole, Mail, Sparkles } from "lucide-react";
  ...
  if (apiConfiguration.baseUrl) {
    return (
      <div className="page-wrap page-enter">
        ...
        <section className="mt-8 grid min-h-[420px] place-items-center rounded-xl border border-border p-8 text-center">
  ...
  return (
    <div className="page-wrap page-enter">
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        ...
        <Button size="lg" onClick={() => setGenerated(true)}><Sparkles className="h-5 w-5" />{locale === "ar" ? "إنشاء حزمة تجريبية" : "Generate demo package"}</Button>
      </div>

      <div className="mt-8 grid gap-8 lg:grid-cols-[280px_minmax(0,1fr)]" dir="ltr">
        <aside className="space-y-3" dir={locale === "ar" ? "rtl" : "ltr"}>
          <h2 className="section-title">{locale === "ar" ? "الحزمة الحالية" : "Current package"}</h2>
          <button onClick={() => setActiveKind("cv")} className={cn("flex min-h-[72px] w-full items-center gap-3 rounded-lg border p-4 text-start", activeKind === "cv" ? "border-emerald bg-emerald-pale" : "border-border hover:border-emerald")}><FileText className="h-6 w-6 text-emerald" /><span><strong className="block text-sm">{locale === "ar" ? "CV مخصص" : "Tailored CV"}</strong><small className="text-muted">{text(targetJob.title)}</small></span></button>
          <button onClick={() => setActiveKind("cover")} className={cn("flex min-h-[72px] w-full items-center gap-3 rounded-lg border p-4 text-start", activeKind === "cover" ? "border-emerald bg-emerald-pale" : "border-border hover:border-emerald")}><Mail className="h-6 w-6 text-emerald" /><span><strong className="block text-sm">{locale === "ar" ? "رسالة تقديم" : "Cover letter"}</strong><small className="text-muted">{locale === "ar" ? "نسخة موجزة" : "Concise version"}</small></span></button>
          <div className="rounded-lg border border-danger bg-danger-pale p-4 text-sm"><p className="flex items-center gap-2 font-semibold text-danger"><LockKeyhole className="h-4 w-4" />{locale === "ar" ? "بيانات خيالية للعرض فقط" : "Fictional data for preview only"}</p><p className="mt-2 text-xs text-danger">{locale === "ar" ? "هذه الحقائق جزء من سيناريو تجريبي وليست حقائقك المهنية. لا تستخدم المستند للتقديم." : "These facts belong to a demo scenario, not your career profile. Do not use this document to apply."}</p></div>
        </aside>

        <section dir={locale === "ar" ? "rtl" : "ltr"}>
          {!generated ? (
            <div className="grid min-h-[480px] place-items-center rounded-xl border border-dashed border-border p-8 text-center"><div><FileCheck2 className="mx-auto h-10 w-10 text-muted" /><h2 className="mt-4 section-title">{locale === "ar" ? "أنشئ الحزمة لمعاينة المستندات" : "Generate the package to preview documents"}</h2><p className="mt-2 text-sm text-muted">{locale === "ar" ? "الإنشاء المحلي يستخدم الحقائق التجريبية المؤكدة فقط." : "Local generation uses only confirmed demo facts."}</p></div></div>
          ) : (
            <>
              ...
              <div className="relative mt-6 min-h-[520px] overflow-hidden rounded-xl border-2 border-danger bg-white p-6 shadow-subtle md:p-10">
                ...
                <div className="border-b-2 border-ink pb-5"><h3 className="text-2xl font-bold">{locale === "ar" ? "محمد العتيبي" : "Mohammed Alotaibi"}</h3><p className="mt-1 text-sm text-muted">{text(targetJob.title)} · {text(targetJob.location)}</p></div>
                {activeKind === "cv" ? <div className="mt-6 space-y-6">{cvSections[locale].map((section) => <section key={section.heading}><h4 className="border-b border-border pb-2 font-bold">{section.heading}</h4><p className="mt-3 text-sm leading-7">{section.text}</p><p className="mt-2 flex items-center gap-1 text-xs text-emerald"><CheckCircle2 className="h-3.5 w-3.5" />{locale === "ar" ? "مرتبط بحقيقة مؤكدة" : "Linked to a confirmed fact"}</p></section>)}</div> : <p className="mt-8 whitespace-pre-line text-sm leading-8">{documentText}</p>}
              </div>
            </>
          )}
        </section>
      </div>
      {notice ? <div className="fixed bottom-24 start-5 z-50 flex items-center gap-2 rounded-lg border border-emerald bg-white px-4 py-3 text-sm shadow-subtle lg:bottom-6" role="status"><CheckCircle2 className="h-4 w-4 text-emerald" />{notice}</div> : null}
    </div>
  );
}
