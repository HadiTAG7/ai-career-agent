...
export function EvidenceDrawer({ requirement, reviewMode = false, evidenceDetail, onClose, open, onCorrectRequirement, onAddRequirement, onRetireRequirement }: EvidenceDrawerProps) {
  ...
  return (
    <aside
      className="fixed inset-x-0 bottom-[72px] top-[86px] z-40 overflow-y-auto border-l border-border bg-white p-5 shadow-subtle lg:sticky lg:inset-auto lg:top-24 lg:z-auto lg:block lg:max-h-[calc(100vh-112px)] lg:shadow-none"
    ...
    >
      <div className="flex items-center justify-between border-b border-border pb-4">
        ...
        <button className="grid h-11 w-11 place-items-center rounded-lg hover:bg-slate-50" onClick={onClose} aria-label={locale === "ar" ? "إغلاق الدليل" : "Close evidence"}><X className="h-5 w-5" /></button>
      </div>

      <section className="open-section">
        <p className="text-xs font-semibold text-muted">{locale === "ar" ? "المتطلب المحدد" : "Selected requirement"}</p>
        <div className="mt-3 rounded-lg border border-emerald p-4">
          <p className="font-semibold">{text(requirement.label)}</p>
          <div className="mt-2">{reviewMode ? <span className="rounded-full bg-amber-pale px-3 py-1 text-xs font-semibold text-amber">{requirement.kind === "essential" ? (locale === "ar" ? "أساسي" : "Essential") : (locale === "ar" ? "مفضل" : "Preferred")}</span> : <StatusLabel status={requirement.status} labels={labels[locale]} compact />}</div>
        </div>
      </section>
      ...
      <section className="open-section">
        ...
        {evidenceDetail ? (
          ...
          <div className="mt-3 rounded-lg border border-danger bg-danger-pale p-4">
      ...
      <section className="open-section">
        ...
        <div className="mt-3 flex items-start gap-2 rounded-lg bg-slate-50 p-4">
        ...
        {onCorrectRequirement ? (
          <div className="mt-3">
            {!editingRequirement ? (
              ...
              <form className="space-y-3 rounded-lg border border-border p-4" onSubmit={handleCorrection}>
